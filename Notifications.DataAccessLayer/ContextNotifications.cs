...
namespace Notifications.DataAccessLayer
{
    public class ContextNotifications2 : DbContext
    {
        ...
        protected override void OnModelCreating(DbModelBuilder modelBuilder)
        {
            ...
            modelBuilder.Entity<SqlReceiversOfNotification>().HasRequired(x => x.Receiver).WithMany(x => x.ReceiveNotifications).HasForeignKey(x => x.ReceiverId).WillCascadeOnDelete(false);
            modelBuilder.Entity<SqlReceiversOfNotification>().HasRequired(x => x.ReceivingNotification).WithMany(x => x.Receivers).HasForeignKey(x => x.NotificationId).WillCascadeOnDelete(false);
